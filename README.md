...
3. In a document, the top row has the page overview, search, sharing, undo, bookmarks, new pages and the read-only switch; the row below holds the tools and their colors and sizes. Inserting a page (iOS: in the page menu, Android: "+ Seite" in the header) adds another PDF's or a picture's pages anywhere in the document, from Fotos or Dateien. "Pip fragen" marks a passage for the tutor. "Neu" → "Notizbuch" in the library starts an empty notebook.
