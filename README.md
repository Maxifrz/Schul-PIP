...
| **Calculating in notes** | Like in Apple's Notes: write a calculation by hand and end it with "=", and the result is offered right after it; a tap writes it in a handwriting font in the pen's color (on Android as a stroke of text in the ink, erasable like handwriting). Values and functions written further up (`a = 5`, `f(x) = x² + 1`) are used. The "Rechnen" tool frames any handwritten region: the recognized lines can be corrected, calculated and written down, a function becomes a graph with its points, and a value table becomes a line or bar chart (on iOS on the page below the frame, on Android as the next page). The vision model set for the tutor only transcribes the handwriting; every number comes from Giac on the device. Without a key the phone's own text recognition reads the handwriting. |
...
| **Kalender** | Stundenplan, Klausurenplan, Ferien and Feiertage in one tab, separate from the AI study plan. The weekly timetable places lessons on a grid colored by the library's subject list, overlapping lessons side by side. The exam list shows every Klausur soonest first, with the calendar days and the actual school days left to prepare (weekends, Feiertage and Ferien do not count). Ferien and Feiertage are bundled offline for all 16 Bundesländer, 2023–2029; the Bundesland is chosen in the app. |
...
│   │               MathNotes (finding a written "=", its line, reading the model's transcription, tables to charts)
...
│   ├── Calendar/   CalendarDay (date-only, Julian-day arithmetic), HolidayCalendar (bundled Ferien/Feiertage),
│   │               Timetable (weekly-grid layout), ExamCountdown
...
calendar/
├── holidays-de.json  Ferien and Feiertage for all 16 Bundesländer, 2023–2029, from openholidaysapi.org
├── fetch.py           re-fetches holidays-de.json from the API
└── generate.py         turns it into the embedded HolidayData.swift and HolidayData.kt
...
├── ink/            Instruments and MathNotes, the same logic as on iOS
...
- **The model reads, the engine calculates.** Handwriting recognition is what vision models are good at and arithmetic is what they are bad at, so the model only transcribes the line (and the lines above it, for definitions) into calculator syntax, and Giac does the maths. A written "=" is found without any model: two short, flat strokes above each other, about as long as each other; the line is the strokes left of it at its height, as long as the gaps stay small. Only after the pen has rested for a moment is the picture sent, so writing on normally costs nothing.
- **Calendar data embedded, not bundled as a resource.** `holidays-de.json` (56 KB) is turned into a Swift and a Kotlin string constant by `calendar/generate.py`, so `HolidayCalendar` loads it with a plain `JSONDecoder`/`Json.decodeFromString` — no `Bundle`, no `context.assets`, no Robolectric asset shadowing to get right. It loads identically in the real app, in a hosted XCTest run and in the Linux SwiftPM harness, and it is the thing the tests actually check: every Bundesland's holidays are cross-checked against the bundled JSON directly rather than against hardcoded dates, so a future `fetch.py` refresh that shifts a date cannot silently go stale.
- **Dates without a time zone.** `CalendarDay` (Swift and Kotlin) holds a year, a month and a day — nothing else — and does its arithmetic through the Julian day number (the Fliegel & Van Flandern conversion), not through `Calendar`/`LocalDate`. Subtracting or adding days this way is exact and gives the same answer regardless of the device's time zone or a daylight-saving change; only "today" reads the device's calendar, once, at the boundary.
...
3. In a document, write a calculation and an "=" with the pen: the result appears as a green button, a tap writes it down ("= rechnet" in the pen's options, on Android in the "Rechnen" menu, turns this off). The "Rechnen" tool (the function symbol) frames handwriting to calculate, graph or chart it. The top row has the page overview, search, sharing, undo, bookmarks, new pages and the read-only switch; the row below holds the tools and their colors and sizes. The ruler button (Android: "Geometrie") lays a ruler, set square, protractor or compass on the page in view and switches to true scale. Inserting a page (iOS: in the page menu, Android: "+ Seite" in the header) adds another PDF's or a picture's pages anywhere in the document, from Fotos or Dateien. "Pip fragen" marks a passage for the tutor. "Neu" → "Notizbuch" in the library starts an empty notebook.
...
8. **Kalender**: "Stunden" builds the weekly timetable — "+ Stunde" picks a subject (or a custom name), room, weekday and times. "Klausuren" lists every exam with the days and school days left; "+ Klausur" adds one. "Ferien" asks for your Bundesland once, then shows the current or next Ferien and the next Feiertage.
...
Covered: SM-2 scheduling, prerequisite ordering and day packing, request encoding and response parsing for both API formats (refusals, truncation, rate limits, missing credits, reasoning tags), tolerant JSON extraction with retry, local PDF extraction and scanned-page detection, on-device OCR (real Vision run plus the fallback order), image compression for NIM, prompt construction, the three-step deck generation and every slide type (fitting, charts, fallbacks, identical layouts on both platforms), library sorting, search, folders and trash, applying chat and critic changes, Wikipedia search and research with citations, exercises and flashcards per topic, iCalendar export (escaping, line folding), reminder texts and schedules, note pages (inserting and deleting pages, shape recognition), PPTX import against python-pptx and LibreOffice files and a round trip through the own writer, .docx reading (headings from styles, bold and italic, bullet and numbered lists, tables, pictures) against real python-docx files, demo content, calendar-day arithmetic (Julian day round trips, known weekdays, leap days, year boundaries), Ferien/Feiertage lookups cross-checked against the bundled data itself, the weekly timetable's overlap layout and exam school-day countdowns, finding a written "=" and its line, reading the model's transcription (JSON or plain lines, school spellings, definitions above the line), tables to chart data, the instruments' geometry (edges and snapping, degrees, protractor rays, compass radius and arcs, turning with two fingers, true scale). On Android, inserting PDF and picture pages was also checked against real PDFs built with PDFBox, splicing the pages in the right place and moving the ink of later pages along.
...
- Calculating in notes needs a clear "=" made of two separate strokes; an "=" written in one stroke is not found. How well handwriting is read depends on the vision model; on-device text recognition reads digits well but formulas poorly, so fractions and powers need a vision model. A result appears only when the pen rests on the "=" for about a second.
...
- Ferien and Feiertage are bundled through 2029; after that, `calendar/fetch.py` needs to be re-run and `calendar/generate.py` re-embeds the data (see `calendar/README.md`). The timetable has no periods or repeating exceptions (like a swapped lesson in one specific week) — just a weekly time, like a paper Stundenplan.
